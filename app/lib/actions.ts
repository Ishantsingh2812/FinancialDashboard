...
export async function authenticate(
  ...
  try {
    ...
    // If successful, redirect to dashboard (or callbackUrl if provided)
    const redirectTo = formData.get('redirectTo') as string | null;
    redirect(redirectTo || '/dashboard/overview');
...
const FormSchema = z.object({
    ...
    amount: z.coerce.number().gt(0,{message:'Please enter an amount greater tthan $0.'}),
...
export async function deleteInvoice(id: string) {
  ...
  try {
    ...
    if (!result || result.length === 0) {
      return { message: 'Invoice not found. Nothing was deleted.' };
    }
